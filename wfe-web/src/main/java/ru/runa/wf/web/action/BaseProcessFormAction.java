...
public abstract class BaseProcessFormAction extends ActionBase {
  ...
  public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
    ...
    try {
      ...
      // user session!)
      if (request.getSession().getAttribute(Globals.TRANSACTION_TOKEN_KEY) == null || isTokenValid(
          request, true)) {
        saveToken(request);
      ...
      if (e.getGlobalErrors().size() > 0) {
        for (String message : e.getGlobalErrors()) {
          if (Strings.isNullOrEmpty(message)) {
            addError(request,
                new ActionMessage(MessagesException.MESSAGE_VALIDATION_ERROR.getKey()));
          } else {
  ...
  protected abstract Long executeProcessFromAction(HttpServletRequest request, ActionForm form,
